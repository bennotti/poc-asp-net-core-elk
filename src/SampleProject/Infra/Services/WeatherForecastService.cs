...
namespace SampleProject.Infra.Services
{
    public class WeatherForecastService : IWeatherForecastService
    {
        ...
        public Task<ListWheatherForecastResponseDto> ObterComError(ListWheatherForecastRequestDto request)
        {
            throw new NotImplementedException();
        }

        public async Task<ListWheatherForecastResponseDto> ObterComErrorTratado(ListWheatherForecastRequestDto request)
        {
            try
            {
                throw new Exception("alguma excessão no sistema");
            }catch(Exception ex)
            {
                _logger.LogError("ObterComErrorTratado", ex);
            }

            return await Task.FromResult(new ListWheatherForecastResponseDto
            {
                Lista = new List<WeatherForecastDto>()
            });
        }
    }
}
